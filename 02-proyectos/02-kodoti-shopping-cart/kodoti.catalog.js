    ...
    render() {
        ...
        this._amount.innerText = (
            this._data.reduce((a, b) => {
                if (a.price) return a.price + b.price;
                return a + b.price;
            }, 0) * this._culture.exchangeRate
        ).toLocaleString(this._culture.code, { style: 'currency', currency: this._culture.currency });
    }
    ...
    render() {
        ...
        function _productTemplate(item) {
            let price = (item.price * self._config.culture.exchangeRate)
                .toLocaleString(self._config.culture.code,
                    {
